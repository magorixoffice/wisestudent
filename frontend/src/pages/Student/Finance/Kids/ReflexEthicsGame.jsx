...
const ReflexEthicsGame= () => {
 ...
 const QUESTIONS_DATA = Array.isArray(gameContent?.questions) ? gameContent.questions : [];
  ...
  const handleAnswer = (option) => {
    ...
    const isLastQuestion = currentRound === QUESTIONS_DATA.length;
  ...
  return (
    <GameShell
      title={gameContent?.title || "Reflex Ethics"}
      subtitle={gameState === "playing" ? (gameContent?.roundLabel?.replace("{{current}}", currentRound).replace("{{total}}", TOTAL_ROUNDS) || `Round ${currentRound}/${TOTAL_ROUNDS}`) : (gameContent?.subtitleComplete || "Test your ethics reflexes!")}
        ...
        {gameState === "ready" && (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 text-center">
            ...
            <p className="text-white/90 text-lg mb-6">
              {gameContent?.readyDescription || "Answer questions about ethical behavior with money!"}<br />
        ...
        {gameState === "playing" && currentQuestion && (
          <div className="space-y-8">
            <div className="flex justify-between items-center bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
              ...
              <div className="text-white">
                <span className="font-bold">{gameContent?.gameScoreLabel || "Score:"}</span> {score}
              </div>
            </div>
