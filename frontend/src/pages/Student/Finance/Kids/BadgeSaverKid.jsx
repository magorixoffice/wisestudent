...
const BadgeSaverKid = () => {
  ...
  const gameContent = t("financial-literacy.kids.badge-saver-kid", { returnObjects: true });
  ...
  return (
    <GameShell
      title={gameContent?.title || "Badge: Saver Kid"}
      subtitle={
        showResult
          ? gameContent?.subtitleComplete || "Badge Earned!"
          : t("financial-literacy.kids.badge-saver-kid.subtitleProgress", {
              current: challenge + 1,
            ...
            {finalScore >= 4 ? (
              <div>
                ...
                <h3 className="text-3xl font-bold text-white mb-4">
                  {gameContent?.resultWinTitle || "Saver Kid Badge Earned!"}
                </h3>
                <p className="text-white/90 text-lg mb-6">
                  {t("financial-literacy.kids.badge-saver-kid.resultWinDescription", {
                    score: finalScore,
                    total: challenges.length,
                  })}
                </p>
                ...
                <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white p-6 rounded-2xl mb-6">
                  ...
                  <p className="text-xl">{gameContent?.achievementBadge || "Badge: Saver Kid"}</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="bg-green-500/20 p-4 rounded-xl">
                    <h4 className="font-bold text-green-300 mb-2">
                      {gameContent?.savingSkillsTitle || "Saving Skills"}
                    </h4>
                    <p className="text-white/90 text-sm">
                      {gameContent?.savingSkillsBody ||
                        "You learned to save regularly, keep money safe, and make smart financial decisions!"}
                    </p>
                  </div>
                  <div className="bg-blue-500/20 p-4 rounded-xl">
                    <h4 className="font-bold text-blue-300 mb-2">
                      {gameContent?.financialWisdomTitle || "Financial Wisdom"}
                    </h4>
                    <p className="text-white/90 text-sm">
                      {gameContent?.financialWisdomBody ||
              ...
              <div>
                ...
                <p className="text-white/90 text-lg mb-4">
                  {t("financial-literacy.kids.badge-saver-kid.resultKeepDescription", {
                    score: finalScore,
                    total: challenges.length,
                  })}
                </p>
                <p className="text-white/90 mb-6">
                  {gameContent?.resultKeepBody ||
                    "Remember, good saving habits include saving regularly, keeping money safe, and setting goals!"}
                </p>
                <button
