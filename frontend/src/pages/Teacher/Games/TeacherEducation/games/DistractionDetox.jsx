...
import { CheckCircle, Circle, Lightbulb, Target, AlertCircle, Clock, Bell, Zap, Battery, Smartphone, Mail, Calendar } from "lucide-react";
...
const DistractionDetox = () => {
  ...
  // Comprehensive list of common distractions
  const allDistractors = [
    ...
    {
      id: 10,
      title: "Open Tabs & Apps",
      description: "Too many browser tabs or applications open, creating visual clutter",
      category: "Environment",
      icon: Smartphone,
    ...
    },
    {
      id: 11,
      title: "Worry & Anxiety",
      description: "Racing thoughts about future tasks or concerns pulling attention away",
      category: "Fatigue",
      icon: AlertCircle,
  ...
  const handleDistractorSelect = (distractorId) => {
    if (selectedDistractors.includes(distractorId)) {
      // Deselect if already selected
      setSelectedDistractors(selectedDistractors.filter(id => id !== distractorId));
    } else if (selectedDistractors.length < 3) {
      // Select if less than 3 are selected
      setSelectedDistractors([...selectedDistractors, distractorId]);
    } else {
      // Already have 3 selected, replace the first one
      setSelectedDistractors([...selectedDistractors.slice(1), distractorId]);
    }
  };

  // Generate personalized Focus Plan based on selected distractors
  const generateFocusPlan = () => {
    const selectedItems = allDistractors.filter(d => selectedDistractors.includes(d.id));
  ...
  const handleGeneratePlan = () => {
    if (selectedDistractors.length === 0) {
      alert("Please select at least one distractor to generate a Focus Plan.");
      return;
    }
        ...
        {!showFocusPlan && !showGameOver && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="mb-6">
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border-2 border-indigo-200 mb-4">
                ...
                <p className="text-gray-700 mb-2">
                  Review the common distractions below. Select your <strong>top 3 distractions</strong> that most reduce your focus quality during work hours.
                </p>
              ...
              <div className="bg-amber-50 border-2 border-amber-200 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-between">
                  <span className="text-gray-700 font-semibold">
                    Selected: {selectedDistractors.length} / 3
                  </span>
                  {selectedDistractors.length === 3 && (
                    <span className="text-sm text-green-700 font-medium flex items-center gap-1">
                      <CheckCircle className="w-4 h-4" />
                      Ready to generate Focus Plan!
                    </span>
                  )}
                </div>
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(selectedDistractors.length / 3) * 100}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-8">
              {allDistractors.map((distractor, index) => {
                const IconComponent = distractor.icon;
                const isSelected = selectedDistractors.includes(distractor.id);
                const isDisabled = selectedDistractors.length >= 3 && !isSelected;
          ...
          >
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border-2 border-indigo-200 mb-8">
              <h3 className="text-lg font-semibold text-gray-800 mb-3">
                Your Top 3 Distractions:
              </h3>
              <div className="flex flex-wrap gap-3">
                {selectedDistractors.map((id) => {
                  const item = allDistractors.find(d => d.id === id);
                  if (!item) return null;
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border-2 border-indigo-200 mb-6">
              ...
              <div className="bg-white/60 rounded-lg p-4 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Distractions Identified</p>
                    <p className="text-2xl font-bold text-indigo-600">{selectedDistractors.length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Focus Strategies</p>
                    <p className="text-2xl font-bold text-purple-600">{focusPlan.length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Action Items</p>
                    <p className="text-2xl font-bold text-green-600">Multiple</p>
                  </div>
                </div>
              </div>
            </div>
